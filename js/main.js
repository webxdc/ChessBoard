...
function receiveUpdate(update) {
    ...
    if (payload.move) {
        if (state.board && update.serial === update.max_serial) {
            ...
            if (lastMove.from !== payload.move.from || lastMove.to !== payload.move.to) {  // the move is not from self
    ...
    } else if (!state.request && payload.request && payload.request === state.whiteAddr) {
    ...
    if (update.serial === update.max_serial) {
        if (!state.blackAddr && state.request && state.selfAddr === state.whiteAddr) {
            state.blackAddr = state.request.addr;
            state.blackName = state.request.name;
            const desc = "Chess: " + normalizeName(state.blackName) + " joined the game";
            const update = {
                payload: {
