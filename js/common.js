...
function hashText(text) {
    return new jsSHA("SHA-512", "TEXT", { encoding: "UTF8" }).update(text).getHash("HEX");
}
...
export function getSummary() {
    const name = normalizeName((state.game.turn() === "w")? state.whiteName : state.blackName);
    ...
    } else if (state.surrenderAddr) { // used by "share" button
        const winner = (state.game.turn() === "b")? normalizeName(state.whiteName) : normalizeName(state.blackName);
